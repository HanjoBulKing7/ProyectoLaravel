...
<head>
    ...
    <title>Editar Categoría - Beauty Bonita</title>
...
<body class="bg-gray-50">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-2xl font-light text-gray-800">Editar Categoría</h1>
            <p class="text-gray-500 mt-1">Modifica la información de la categoría</p>
        </div>
        ...
        <!-- Formulario -->
        <div class="card p-6">
            <form action="{{ route('admin.categoriaservicios.update', $categoria->id_categoria) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="descripcion" id="descripcion" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-gray-300">{{ old('descripcion', $categoria->descripcion) }}</textarea>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        @if($categoria->imagen)
                            <img src="{{ asset('storage/' . $categoria->imagen) }}" alt="{{ $categoria->nombre }}" class="h-20 w-20 object-cover rounded-md mb-2">
                            <p class="text-sm text-gray-500">Imagen actual</p>
                        @else
                            <div class="h-20 w-20 bg-gray-200 rounded-md flex items-center justify-center mb-2">
                                <i data-feather="image" class="text-gray-400"></i>
                            </div>
                            <p class="text-sm text-gray-500">No hay imagen</p>
                        @endif
                    </div>

                    <!-- Nueva imagen -->
                    <div class="md:col-span-2">
                        <label for="imagen" class="block text-sm font-medium text-gray-700 mb-1">Nueva imagen (opcional)</label>
